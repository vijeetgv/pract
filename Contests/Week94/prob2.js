...
var robotSim = function(commands, obstacles) {
    ...
    if(commands!=null && obstacles!=null){
        ...
        for(var i=0; i<obstacles.length; i++){
            var yMap = obstMap[obstacles[i][0]];
            if(!yMap){
                obstMap[obstacles[i][0]] = yMap = {};
            }
            yMap[obstacles[i][1]] = true;
        }
        var dirIdx = 0;
        var turnsOrd = [[0,1], [1,0], [0,-1], [-1,0]];
        var direction = turnsOrd[dirIdx];

        var changePos = function (obstMap, direction, pos, m) {
            if(m){
                var x = pos.x;
                var y = pos.y;
                for(var i=0; i< m; i++){
        ...
        for(var i=0; i<commands.length; i++){
            ...
            switch (c){
                ...
                default: {
                    changePos(obstMap, direction, pos, c);
                    ed = Math.max(ed, (pos.x * pos.x + pos.y * pos.y));
                }
            }
        }
    }
